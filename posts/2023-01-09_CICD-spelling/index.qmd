...
-   At last, we run the spell check in R.
    ...
    The same goes for the `ìnst/WORDLIST` file.
...
This package comes with the english dictionary preinstalled.
...
As soon as we want to spell-check on GitHub (with CICD) it gets a bit more tricky because we need to reference a library file.
