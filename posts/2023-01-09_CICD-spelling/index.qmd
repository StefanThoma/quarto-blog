...
You can find a more comprehensive guide to the `spelling` package [in the manual.](https://cran.r-project.org/web/packages/spelling/spelling.pdf)
...
To always get such an object we simply spell-check the `WORDLIST_EXAMPLE.txt` file using itslef as the list of words to ignore:
...
jobs:
  Spelling:
    ...
    steps:
      ...
      - name: test typos
        run: testthat::test_that(desc = "No Typo", code = testthat::expect_equal(object = spelling::spell_check_files(list.files(pattern = ".*.qmd$", recursive = TRUE), ignore = readr::read_lines("inst/WORDLIST.txt"), lang = "inst/de_CH_FRAMI"), expected = spelling::spell_check_files(path = "inst/WORDLIST.txt", ignore = readr::read_lines("inst/WORDLIST.txt"), lang = "inst/de_CH_FRAMI")))
        shell: Rscript {0}
```
